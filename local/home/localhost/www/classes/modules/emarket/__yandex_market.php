...
require_once SYS_KERNEL_PATH . 'utils/yandex_market/vendor/autoload.php';
			...
			if ($event->getMode() == "before") {
				...
				if (isset($modifiedCache[$object->getId()])) {
					...
					if ($modifiedCache[$object->getId()]['name'] === 'status_id') {
						...
						try {
							$this->_sendStatusOnChangeOrderStatus($orderId, $status);
						} catch (Exception $e) {
						...
						$deliveryTypeObject = delivery::get($delivery->id);
						//try {
						$deliveryPrice = (float)$deliveryTypeObject->getDeliveryPrice(order::get($object->id));
						/*} catch (Exception $e) {
						$deliveryPrice = 0;
						}*/
