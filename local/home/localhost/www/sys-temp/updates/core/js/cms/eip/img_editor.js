uAdmin('.ieditor', function (extend) {
	...
	var uImageEditor = function () {
		...
		jQuery("body").on('click', function(oEvent){
			if (jQuery(oEvent.target).closest("." + settings.collection.editor_id_attribute + ", ." + settings.img_area_select.css_class).length) return;
			if (jQuery(oEvent.target).is(".mce-resizehandle, img")) return;
	...
	uImageEditor.getNodes = function (bReturnJquery) {
		var jqNodes = jQuery("img").filter(function(){
			return jQuery(this).is("[umi\\:field-name]") || jQuery(this).parents(".mceEditor").length || jQuery(this).parents("[umi\\:field-type='wysiwyg']").length;
		});
		return bReturnJquery ? jqNodes : jqNodes.toArray();
	};

	uImageEditor.prototype.getNodes = function (bReturnJquery) {
		return uImageEditor.getNodes(bReturnJquery);
	};
	...
	ImageEditorBase.prototype.init = function (jqImgNode, sEditorId) {
		...
		if (jqImgNode.parents("[umi\\:module='photoalbum']").length) {
			this.iImageType = this.IMAGE_TYPE_PHOTOALBUM;
		} else if (jqImgNode.is("[umi\\:field-name]")) {
			this.iImageType = this.IMAGE_TYPE_EIP;
		} else if (jqImgNode.parents(".mceEditor").length || jqImgNode.parents("[umi\\:field-type='wysiwyg']") || jqImgNode.parents(".mce-content-body").length) {
	...
	ImageEditorBase.prototype.hidePreloader = function (bIgnoreSwitchOn) {
	...
	ModuleBase.prototype.setNewImage = function (sNewImageSrc, resetCache) {
		...
		if (this.oEditor.iImageType == this.oEditor.IMAGE_TYPE_WYSIWYG) {
			if (PopupModule && PopupModule.changeImage) {
				PopupModule.changeImage(this.oEditor, sNewImageSrcNoCache);
			}
			if (!sNewImageSrc) {
		...
		this.oEditor.jqImgNode.on("load", function(){
			...
			var img = this;
			if (self.oEditor.jqImgNode && self.oEditor.jqImgNode.length) {
				self.oEditor.jqImgNode.removeAttr('width').removeAttr('height');
				self.oEditor.jqImgNode[0].style.width = '';
				self.oEditor.jqImgNode[0].style.height = '';
			}
			self.oEditor.reinit();
			window.setTimeout(function(){
				uAdmin.eip.normalizeBoxes();
	...
	CropModule.prototype.save = function () {
		...
		this.oEditor.jqImgNode.removeAttr("width height");
	...
	CropModule.prototype.drawOriginalImage = function () {
		...
		this.jqOriginalImage = jQuery("<img/>")
			...
			.one("load", function(){
				...
				self.repositionMenu();
				self.oEditor.hidePreloader(true);
			});
		this.imgAreaSelectOptions.parent = this.jqImgAreaSelectWrapper;
	};
	...
	PopupModule.changeImage = function (oEditor, sNewImage) {
		if (oEditor instanceof ImageEditorBase === false) return;
	...
	DeleteModule.prototype.activate = function () {
		...
		switch (this.oEditor.iImageType) {
			...
			case this.oEditor.IMAGE_TYPE_PHOTOALBUM:
				info = this.oEditor.getEipInfo();
				info['delete'] = true;
				uAdmin.eip.queue.add(info);
				uAdmin.eip.normalizeBoxes();
				break;
