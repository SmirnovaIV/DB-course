...
class menu extends def_module {
	...
	private function drawLevel($arr=NULL,$templates,$curr_depth=0,$allParents) {
		...
		) = $this->getMenuTemplatesTpl($templates, ($curr_depth + 1));		
	...
	public function draw($menuId = NULL, $template = "default") {
		...
		$menu = $uOC->getObject($menuId);
		if ($menu instanceof umiObject) {			
			$menuHierarchy = $menu->menuHierarchy;
			if(!$menuHierarchy) throw new publicException(getLabel('error-prop-not-found',null,'menuHierarchy'));
