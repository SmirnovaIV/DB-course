	...
	abstract class __editor_content {
		...
		public function eip_add_page() {
			...
			if($mode == 'choose') {
				...
				if(sizeof($types) == 1) { //Redirect to next step
					...
					$url = $this->pre_lang . "/admin/content/eip_add_page/form/{$parentId}/{$module}/{$method}/?csrf={$csrf}";
		...
		public function getImageData ($sImagePath = '') {
			...
			if (!file_exists($sImagePath)) {
				throw new coreException("Invalid file name");
			}
			$arResult = array(
		...
		public function ieditor ($sAction) {
			...
			if (str_replace(CURRENT_WORKING_DIR, '', $sImagePath) == getRequest('empty_url') && $sAction != 'upload') {
				throw new coreException("This image can not be edited");
			}
